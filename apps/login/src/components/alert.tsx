...
const red =
  "border-alert-error-foreground bg-alert-error-background text-alert-error-foreground";
const neutral =
...
export function Alert({ children, type = AlertType.ALERT }: Props) {
  return (
    <div
      className={clsx(
        "flex flex-row items-center justify-center border rounded-md py-3 px-4 scroll-px-40",
        {
    ...
    >
      {type === AlertType.ALERT && (
        <ExclamationTriangleIcon className="flex-shrink-0 h-5 w-5 mr-3" />
      )}
      {type === AlertType.INFO && (
        <InformationCircleIcon className="flex-shrink-0 h-5 w-5 mr-3" />
      )}
