...
// Route-level loader test for the OTP-email LINK fix (Bug A):
//   - arriving WITHOUT ?code sends the challenge once and returns code: '';
//   - arriving WITH ?code prefills the form AND suppresses the duplicate challenge re-send.
...
describe('login.verify.email loader — OTP-email LINK fix (Bug A) route gating', () => {
  it('sends the otpEmail challenge once on first arrival (typed-in path, no ?code)', () => {
    callService({
  ...
  it('returns an empty code when arriving WITHOUT a ?code (typed-in path)', () => {
    callService({
      fn: 'loginVerifyEmailLoader',
      provider: 'singleton',
      liveSessions: [LIVE_SESSION],
      ...
      expect(v.response?.dataBody).to.have.property('code', '');
    });
  });

  it('prefills the code from ?code when arriving via the emailed link', () => {
    callService({
      fn: 'loginVerifyEmailLoader',
      provider: 'singleton',
      liveSessions: [LIVE_SESSION],
      ...
      expect(v.response?.isResponse).to.equal(false);
      expect(v.response?.dataBody).to.have.property('code', '86230120');
    });
  });
