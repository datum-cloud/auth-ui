...
describe('FakeAuthProvider — MFA (P5)', () => {
  ...
  it('updateSession with empty otpEmail is a no-op — factor NOT verified, no challenge returned', async () => {
    ...
    // No code returned for empty string (use checks.challenges.otpEmail instead)
    expect(s2.challenges?.otpEmailCode).toBeUndefined();
  });
  ...
  it("updateSession with challenges.otpEmail { kind: 'return-code' } requests challenge — factor NOT verified", async () => {
    ...
    const s = await p.createSession({ password: 'pw' }, { userId: 'u1' });
    // Discriminated challenge — return-code delivery surfaces the code in-band.
    const s2 = await p.updateSession(s.id, s.token, {
      challenges: { otpEmail: { kind: 'return-code' } },
    });
    // Factor must NOT be verified (a challenge request does not complete the factor)
    expect(s2.factors.otpEmail).toBeUndefined();
    // The returned code must be present on the returned session
    expect(s2.challenges?.otpEmailCode).toBeDefined();
    ...
    expect(stored?.challenges?.otpEmailCode).toBeUndefined();
  });
