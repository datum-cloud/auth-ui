...
describe('toSession (proto → neutral)', () => {
  ...
  it('converts proto Timestamps to ISO strings (never "[object Object]")', () => {
    ...
    expect(s.factors.password?.verifiedAt).toEqual(new Date('2023-11-14T22:13:20.000Z'));
...
it('toSession maps a verified TOTP factor from the real proto shape', () => {
  ...
  expect(session.factors.totp?.verifiedAt).toEqual(new Date('2026-06-14T00:05:00.000Z'));
});
...
it('toSession maps verified otpEmail / otpSms / u2f factors', () => {
  ...
  const session = toSession(proto, 'tok');
  expect(session.factors.otpEmail?.verifiedAt).toEqual(new Date('2026-06-14T01:00:00.000Z'));
  expect(session.factors.otpSms?.verifiedAt).toEqual(new Date('2026-06-14T02:00:00.000Z'));
  expect(session.factors.u2f?.verifiedAt).toEqual(new Date('2026-06-14T03:00:00.000Z'));
});
