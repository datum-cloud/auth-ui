...
describe('signupMethodSchema', () => {
  it('accepts a valid method intent', () => {
    expect(signupMethodSchema.safeParse({ intent: 'email-link', loginName: 'a@b.com', firstName: 'A', lastName: 'B' }).success).toBe(true);
  });
  it('rejects an unknown intent', () => {
    expect(signupMethodSchema.safeParse({ intent: 'sms', loginName: 'a@b.com', firstName: 'A', lastName: 'B' }).success).toBe(false);
  });
});
