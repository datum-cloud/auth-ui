...
describe('verifyUrlTemplate', () => {
  ...
  it('includes the /id app basename so the link lands on auth-ui, not the Zitadel API', () => {
    // The app is served under the /id/ basename; the origin (PUBLIC_ORIGIN) is shared with the
    // Zitadel API, so a link WITHOUT /id hits Zitadel (NOT_FOUND) instead of our route.
    ...
    expect(verify.startsWith('https://auth.localtest.me:30000/id/verify?')).toBe(true);
    expect(reset.startsWith('https://auth.localtest.me:30000/id/password/new?')).toBe(true);
  });
  ...
  it('uses the passed origin VERBATIM, including its scheme (no hardcoded https)', () => {
    ...
    expect(t.startsWith('http://localhost:3000/id/verify?')).toBe(true);
  ...
  it('defaults the path to /verify (under /id) when none is supplied', () => {
    ...
    expect(t.startsWith('https://auth.datum.net/id/verify?')).toBe(true);
  });
  ...
  it('threads requestId onto an overridden path (URL-encoded)', () => {
    ...
    expect(t.startsWith('https://h/id/password/new?')).toBe(true);
    expect(t).toContain('&requestId=a%20b%2Fc');
  });
});
