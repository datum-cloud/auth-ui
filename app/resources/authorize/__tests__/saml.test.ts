// Pass 2 service test (migrated from routes/authorize/__tests__/saml.test.ts).
...
//
// The branch is STATELESS: /authorize no longer calls createSamlResponse, resolves the
// binding, or sets a saml_form cookie. It validates the request, gates on a session, then hands
// off to the BFF /sso/saml-post handler. Binding-specific + cookie assertions live in
// app/server/routes/saml-post.test.ts. The branch logic now lives in resolveAuthorize; we drive
// the service and translate the outcome to a Response (identical to the old route loader).
...
/** Drive the extracted service exactly as the route does and return the Response. */
async function runLoader(req: Request) {
  const outcome = await resolveAuthorize(fake, req);
  return outcomeToResponse(outcome, new URL(req.url));
}
