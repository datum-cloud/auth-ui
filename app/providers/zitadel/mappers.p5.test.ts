import type { SessionChecks } from '../auth-provider';
import {
  durationToMs,
  mapAuthMethod,
  mfaInitSkippedToIso,
...
describe('toSessionChallenges', () => {
  ...
  it('maps otpEmail string value', () => {
    const result = toSessionChallenges({ otpEmail: '123456' });
    expect(result.otpEmail).toBe('123456');
  ...
  it('maps all three challenge types simultaneously', () => {
    ...
    expect(result.webAuthN?.publicKeyCredentialRequestOptions).toBe(pkOptions);
    expect(result.otpEmail).toBe('111111');
  ...
  it('omits webAuthN when it is null', () => {
    ...
    expect(result.webAuthN).toBeUndefined();
    expect(result.otpEmail).toBe('000000');
  });

  it('omits otpEmail when it is null', () => {
...
describe('toChallengeRequest — otpEmail url_template', () => {
  it('sends an EMPTY sendCode value when otpEmail is bare true (no template)', () => {
    const result = toChallengeRequest({ otpEmail: true });
    expect(result.otpEmail).toEqual({ deliveryType: { case: 'sendCode', value: {} } });
  });

  it('sets url_template on the sendCode value when otpEmail carries a urlTemplate', () => {
    ...
    const result = toChallengeRequest({ otpEmail: { urlTemplate: tmpl } });
    expect(result.otpEmail).toEqual({
      deliveryType: { case: 'sendCode', value: { urlTemplate: tmpl } },
    });
  });

  it('falls back to an empty sendCode value when otpEmail is an object without urlTemplate', () => {
    const result = toChallengeRequest({ otpEmail: {} });
    expect(result.otpEmail).toEqual({ deliveryType: { case: 'sendCode', value: {} } });
  });
