...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  // Session.challenges (not used by this loader but the send is the side effect we want).
  try {
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  let session;
  try {
    session = await provider.updateSession(entry.id, entry.token, { otpSms: code });
  } catch (err) {
    logAuthEvent('mfa_otp', 'failure', { loginName, channel: 'sms' });
