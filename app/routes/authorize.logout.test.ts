// app/routes/authorize.logout.test.ts
...
//
// These cases drive the REAL loader against the shared fake provider singleton, using the
// fake's additive test-control hooks (seedLiveSession / setSessionResult / setCallbackResult)
// to script getSession / createCallback per session id. All hooks are cleared in afterEach so
// the shared singleton stays clean for sibling test files.
