...
import { otpDeliveryCodeSchema } from '@/flows/mfa-schemas';
import { otpEmailUrlTemplate } from '@/flows/otp-email-url-template';
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  // (/id/login/verify/email) instead of its default /ui/v2/login/otp/email page. The result
  // rides back on Session.challenges (unused here — the send is the side effect we want).
  if (!code) {
    try {
      const provider = providerForRequest(request);
      const urlTemplate = otpEmailUrlTemplate({
  ...
  return data({ csrfToken, loginName, requestId, organization, code }, { headers });
}
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const parsed = z
    .object({
      code: otpDeliveryCodeSchema.shape.code,
  ...
  let session;
  try {
    session = await provider.updateSession(entry.id, entry.token, { otpEmail: code });
  } catch (err) {
    logAuthEvent('mfa_otp', 'failure', { loginName, channel: 'email' });
...
export default function VerifyEmail() {
  const { csrfToken, loginName, requestId, organization, code } = useLoaderData<typeof loader>();
